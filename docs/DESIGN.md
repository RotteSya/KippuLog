...
| timeline → detail | one shot: the ticket lifts off its printed place (the slot waits EMPTY — captions stay, the object is simply gone) while the whole spread turns away behind its spine like a page of the book, revealing the dark room that always lived beneath the paper; the ticket rises into the lamp; once the exhibit holds still, the placard lays itself out — catalogue line first, then the route draws itself as a line of track (the arrival dot presses in 朱), the fare, and finally the 旅の記 slip rises into place |
| timeline pinch | the pinch holds the roll itself (ShelfFold): the issue winds up onto a cylinder of its own paper that sweeps the diagonal from the bottom-trailing corner — rolled like a real magazine, never swung like a door. The roll is drawn per frame (crest light, silhouette rim, paper-thickness edge, the shadow it drags), the live page slides under its contact line behind a diagonal mask, and the 収蔵帳 is simply *uncovered* beneath, already complete; release carries the SAME roll through the commit spring. The two shelves never crossfade |
| timeline ⇄ 収蔵帳 | one continuous roll both ways — pinch, corner doors, and month stamps all drive the same engine; the月 jump scrolls under the still-rolled page so the uncovering opens onto the month |
| launch | the launch screen is blank paper; the issue prints itself onto it — masthead first, the catalogue after, the punch stepping in last (one cascade clock) |
...
| detail dismiss | the same shot reversed: the spread swings back in first (flat by the flight's halfway mark) with its slot still empty, the ticket descends onto the waiting page, and the object refills its printed place the exact frame it lands — one ticket, one place, never two. Squeezing the stage scrubs the page live: the return begins in the fingers |
